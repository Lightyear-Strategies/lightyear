...
def receive_journalists():
        ...
        if files:
            ...
            if not db.inspect(db.engine.connect()).has_table(f'journalists{timeframe}'):
                ...
                try:
                    ...
                    email_html_tracker(user_name,user_email,timeframe2freq(timeframe))
                    mp.track(session['email'], 'Subscribed to Journalist Tracker', {'Frequency': timeframe[1:] if timeframe else 'UNKNOWN', 'session_id': request.cookies.get('session')})
    ...
    mp.track(session['email'], 'Viewed Journalist Tracker', {'session_id': request.cookies.get('session')})
...
def unsubscribe_journalist(token):
    ...
    mp.track(session['email'], 'Unsubscribed from Journalist Tracker', {'Frequency': timeframe[1:] if timeframe else 'UNKNOWN', 'session_id': request.cookies.get('session')})
